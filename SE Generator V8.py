...
jcc_network_drive_folder = "S:/All Technical & Support Files/Dictionaries"
home_network_drive_folder = "/Volumes/home/Dictionaries"
corp_network_folder = "C:/Users/cbrichardson/Documents/SEGeneratorDictionary.json"



print("Initializing libraries")
# Initialize SymSpell library and set dictionary path to use
sym_spell = SymSpell(max_dictionary_edit_distance=3, prefix_length=7)
sym_spell_corrections = SymSpell()
filename = "dictionary.json"
frequency_count = 10000
#filename = "bigram.json"
#filename = "testdict.json"
home_directory = os.path.expanduser("~")
documents_directory = os.path.join(home_directory, "Documents")
dictionary_path = os.path.join(home_network_drive_folder, filename)
correction_dictionary_path = "/Volumes/home/Dictionaries/dictionarytestupdate.json"
#dictionary_path = pkg_resources.resource_filename("symspellpy", "frequency_dictionary_en_82_765.txt")
#bigram_path = pkg_resources.resource_filename("symspellpy", "frequency_bigramdictionary_en_243_342.txt")

print("Loading dictionary, this may take a few minutes")
#open dictionary as json, goal is improving load speed as python will load a .json faster than a .txt
...
#handle options within the Spell Check window
...
#WE SHOULD CHECK SOMEWHERE IF MORE THAN JUST THE FIRST WORD IS THE "MISSPELLED WORD"
#This also needs updated for capitalization check and punctuation check
def spellcheck():
...
def request_initial_settings(self):
    ...
    if os.path.exists('ServiceEventGeneratorSettings.ini'):
        ...
        if config.has_option(main_section, 'welcome_message'):
            print("We have a welcome message setting saved")    
        ...
        if config.has_option(main_section, 'extension'):
            ...
            #extensionEntry.insert(1, ext)
            AgentSettings.extension.set(ext)
        #print("we ended this loop and didnt request settings")
        
        else:
            print('We requested settings')
            settings_request_window = tk.Toplevel(root)
            settings_request_window.attributes('-topmost', True)
            #settings_request_window.iconbitmap("jha.ico")
...
def welcome_message(setting):
    ...
    if os.path.exists(ini_file_path):
        ...
        if config.has_section('main'):
            print("We have a main section")
            if config.get('main', 'welcome message') == 'no':
                return
            #if setting != 1 and not config.getboolean('main', 'welcome message'):
                print("We should have exited")
                return
...
class MainApplication(tk.Frame):
    ...
    # - Initialize the GUI - #
    def __init__(self, master, program_settings, SE_smartsheet, *args, **kwargs):
        self.master = master
        ...
        #    'Bank or Credit Union?': f'{AgentSettings.agentType.get()}',
        ...
        #    'CIT/Account Number/Member Number' : f'{self.reference_number.get()}', 
        #    #need to adjust this for a phone number?
        #    'Call Back Number' : f'{self.contact_info.get()}', 
        #    'Service Event Details' : f'{self.reason_entry.get(1, tk.END)}', 
        #    'Customer Name' : f'{self.customer_name.get()}'
        #    }   
        tk.Frame.__init__(self, self.master, *args, **kwargs)
        SE_smartsheet = SE_smartsheet
        #self.agentsettings = AgentSettings(self)
        #self.filemenu = FileMenu(self)
        #self.programsettings = ProgramSettings(self)
        #self.clearundo = ClearUndo(self)
        MainApplication._instance_counter = 0
        MainApplication._last_radio_value = str("Phone")
        #self.check_phone_digits_cmd = (self.register(self.restrict_contact_field))
    ...
    def configure_gui(self):
        ...
        #self.master.iconbitmap("jha.ico")
        self.master.option_add('*tearOff', False)
        #reg = root.register(format_phone_number)
        #self.reference_number_entry.config(validate = "key", validatecommand = (reg, '% P'))
        #self.event_entry.insert(tk.END, self.combined_text)
    ...
    def create_bindings(self):
        ...
        #self.phone_radio_button.bind("<<ButtonSelected>>", print('test'))
        ...
        #self.reason_entry.bind("<KeyRelease>", lambda event: [self.combine_text(None), on_text_changed])
        ...
        self.reason_entry.bind("<ButtonRelease-2>", on_right_click)
        ...
        for i, widget in enumerate(self.widgets):
            ...
            self.customer_name_entry.focus_set()  # Set initial focus to the first entry field
        #self.contact_info_entry.bind("<KeyRelease>", lambda event: self.combine_text(None))
        #self.email_radio_button.bind("<<RadioButtonSelected>>", lambda: self.contact_update)


# - Operation Functions - ##  

    def focus_next(self, widgets, current_index, event = None):
        print(widgets, current_index)
...
#THIS FUNCTION MAY NOT BE NEEDED AS USER NOW HAS TO FILL OUT SETTINGS FILE ON FIRST TIME LAUNCH OF NO SETTINGS
    ...
    def combine_text(self, event=None):
        ...
        if AgentSettings.agentType.get() == 'Bank':
            self.combined_text = ""
            reason = self.reason_entry.get('1.0', tk.END)
            transactions = self.transactions_entry.get('1.0', tk.END)
    ...
    #        # Check if the main section contains the 'agent type' key
    #        if config.has_option(main_section, 'agent_type'):
    #            print('we set agent type successfully')
    #            agent_type = config.get(main_section, 'agent_type')
    #            AgentSettings.agentType.set(agent_type)
...
class AgentSettings():
    ...
    def configure_window(self):
        ...
        #self.settings_request_window.iconbitmap("jha.ico")
    ...
    def request_settings(self):
        ...
        if os.path.exists('ServiceEventGeneratorSettings.ini'):
            ...
            if self.config.has_option(main_section, 'extension'):
                ...
                #self.extensionEntry.insert(1, ext)
                #AgentSettings.extension.set(ext)
            #self.settings_request_window.destroy()
        ...
        #with open('ServiceEventGeneratorSettings.ini', 'w') as configfile:
...
class FileMenu(tk.Menu):
    ...
    def __init__(self, master, main_app, program_settings, agent_settings):
        self.master = master
        ...
        #self.agent_setting = AgentSettings(self)
        ...
        #self.fi_select_menu.bind("<Button-1>", lambda: self.update_name)


# - Initialize the program settings from the settings file if one exists, creates a new one if not - #
class ProgramSettings():
    
    def __init__(self, master, *args, **kwargs):
        self.master = master
        #self.config=ConfigParser()
        #file_menu = FileMenu(self.master, main_app=self, program_settings=self, agent_settings=self)
        #self.master.config(menu=file_menu.menu)
        #self.open_agent_settings_if_no_settings_file()

    def open_agent_settings_if_no_settings_file(self):
        if not os.path.exists('ServiceEventGeneratorSettings.ini'):
            self.agent_settings = AgentSettings(self.master)


    #Check to see if there is a settings file already, if there is load the settings, if not return False
    def check_settings(self, master):    
        if os.path.isfile('ServiceEventGeneratorSettings.ini'):
            self.config=ConfigParser()
            self.config.read('ServiceEventGeneratorSettings.ini')
            ProgramSettings.agentType=self.config.get('main', 'Agent Type')
            extension=self.config.get('main', 'extension')
            settings= Toplevel(master)
            settings.geometry("300x200")
            settings.title("Agent Settings")
            #settings.iconbitmap("jha.ico")
            tk.Label(settings, text= "Current agent settings loaded, please review for accuracy and update if needed.", font=('Poppins 12 bold'), wraplength=300).place(x=15,y=5)
            tk.Label(settings, text= f"Agent Type = {ProgramSettings.agentType}", font=('Poppins 12')).place(x=15, y=100)
            tk.Label(settings, text= f"Extension = {extension}", font=('Poppins 12')).place(x=15, y=150)
        else:
            return False
...
class CustomSmartsheet(smartsheet.Smartsheet):
    ...
    def __init__(self, master, api_token):
        ...
        #self.Sheets = smartsheet
        self.master = master
        self.sheet_id = 2320617425921924
        

    def send_event (self, event, sheet_id, row_values):
        #CustomSmartsheet.smartsheet_event_options(self)
        ...
        #print("Row added with row ID:", result.data[0].id)
...
#Will need popup window form for these fields: Inquiry or Call Back?, FI depending on Agent Type (Bank or CU), 
    ...
    def smartsheet_event_options(self, event, sheet_id):
        print(self.start_time, self.current_time, self.run_flag)
        ...
        if self.check_if_time_elapsed(self, self.start_time, duration):
            print(f"These are our event details {self.event_details_copy}")
        ...
        #self.smartsheet_event_window.attributes('-topmost', True)
        #self.smartsheet_event_window.iconbitmap("jha.ico")
        ...
        if AgentSettings.agentType.get() == 'Credit Union':
            #print(self.get_picklist_values(self.sheet_id, 'Credit Unions'))
            ...
            #print(self.get_picklist_values(self.sheet_id, 'Banks'))
            #return fi_list
        ...
        #print(CustomSmartsheet.fi_list)
        ...
        #if AgentSettings.agentType == 'CU':
        #    AgentSettings.agentType == 'Credit Union'
        print(AgentSettings.agentType)
        if AgentSettings.agentType.get() == 'Credit Union':
            print("cu was called", CustomSmartsheet.selected_fi.get())
            CustomSmartsheet.row_values = {
                'Agent Name': 'Chris Richardson', #this can be formula'ed from extension
                'Agent Extension': f'{AgentSettings.extension.get()}',
                'Bank or Credit Union?': f'{AgentSettings.agentType.get()}',
        ...
        print(f"These are our event details at the end: {self.event_details_copy}")
        print(CustomSmartsheet.selected_fi.get())
...
if __name__ == '__main__':
    ...
    program_settings = ProgramSettings(root)
    SE_smartsheet = CustomSmartsheet(root, api_token)
    main_app = MainApplication(root, program_settings=program_settings, SE_smartsheet=SE_smartsheet)
    file_menu = FileMenu(root, main_app=main_app, program_settings=program_settings, agent_settings=agent_settings)
    welcome_message(None)
    #img =tk.PhotoImage(img.open("jha.gif"))
